...
const deploymentMode =
  process.env.DEPLOYMENT_MODE || (isVercel || !hasDbConfig ? "demo" : "local");
...
async function testConnection(retries = 3, delay = 2000) {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.log(
        `Successfully connected to DB pool (${deploymentMode} mode).`,
      );
      conn.release();
      return true;
    } catch (err) {
...
if (deploymentMode !== "demo") {
  setInterval(async () => {
    ...
    try {
      ...
      if (
        activeConnections > MAX_WARNING_CONNECTIONS &&
        !connectionWarningShown
      ) {
        console.warn(
          `⚠️ High connection count: ${activeConnections}/${pool.config.connectionLimit}`,
        );
